...
**Plot prior sensitivity analysis**
```{r}
...
**Function for simulating with different values for bias**
```{r}
list_of_dfs <- list()
temp <- list()
recovery_df <- data.frame()

for(bias_lvl in seq(0, 1, 0.2)){
  set.seed(1982)
  ...
  # Checking the model's chains (noise)
  temp = ggplot(draws_df, aes(.iteration, bias, group = .chain, color = .chain)) +
      geom_line() +
      theme_classic()
  
  # assign function within loop
  assign(paste0("plot_", bias_lvl), temp)

  assign(paste0("draws_df_", bias_lvl), draws_df)
...
**Plots for different values of bias**
```{r}
plot_0.2

# add a prior for theta (ugly, but we'll do better soon)
draws_df_0.2<- draws_df_0.2 %>% mutate(
  bias_prior = rbeta(nrow(draws_df_0.2), 1, 1)
)
...
# add a prior for theta (ugly, but we'll do better soon)
draws_df_0.4<- draws_df_0.4 %>% mutate(
  bias_prior = rbeta(nrow(draws_df_0.4), 1, 1)
)
...
# add a prior for theta (ugly, but we'll do better soon)
draws_df_0.6<- draws_df_0.6 %>% mutate(
  bias_prior = rbeta(nrow(draws_df_0.6), 1, 1)
)
...
# add a prior for theta (ugly, but we'll do better soon)
draws_df_0.8 <- draws_df_0.8 %>% mutate(
  bias_prior = rbeta(nrow(draws_df_0.8), 1, 1)
)
...
### Actual recovery Plots

**Function for simulating with different values for beta and bias**
...
# Recovery plot - Recovered beta values for different biases
```{r}
...
ggplot(recovery_df_2, aes(betaTrue, betaEst)) +
  ...
  geom_smooth(method = 'lm') +
  facet_wrap(.~ biasTrue) +
  theme_classic() + ggtitle("Parameter recovery for various bias and beta levels - 120 trials")
...
**Function for simulating with different values for alpha and bias**
...
for (bias_lvl in seq(0, 1.5, 0.5)){
  for(alpha_lvl in seq(0.2, 0.8, 0.2)){
    ...
    temp2 <- tibble(alphaEst = (draws_df$alpha),
                    alphaTrue = alpha_lvl,
                    biasTrue = bias_lvl)
...
# Recovery plot - Recovered alpha values for different biases
```{r}

ggplot(recovery_df_2, aes(alphaTrue, alphaEst)) +
  geom_point(alpha = 0.01) +
  geom_smooth(method = 'lm') +
  facet_wrap(.~ biasTrue) +
  theme_classic() + ggtitle("Parameter recovery for various bias and alpha levels - 120 trials")
```
