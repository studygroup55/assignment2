...
data {
  ...
  vector<lower=0, upper=1>[n_trials] next_choice; // based on other's actions, 1s for choosing right next time and -1 for choosing left next time
...
parameters {
  real<lower=0, upper=1> rate; // the rate of choosing left or right
...
transformed parameters {
  ...
  // rate_w_noise = rate + right_choice*noise;
  vector[n_trials] rate_w_noise; // the parameter of the rate and noise
  if (noise == 0) {rate_w_noise = (rate + next_choice)/2;}
  else {rate_w_noise = (rate + next_choice*(1-(1/noise)))/2;}
}

model {
  target += normal_lpdf(rate | 0, 1); // normally distributed rate
