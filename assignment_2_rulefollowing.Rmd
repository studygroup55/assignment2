...
WSLSAgent_f <- function(prevChoice, Feedback, ruleFollowing) {
  ...
  if(rbinom(1,1, 1-ruleFollowing)==1){
  choice <- rbinom(1,1,0.5)
  }
  
  else if (Feedback == 1) {      #If feedback = 1 (win), stay
    choice = prevChoice
  }
...
```{r}
for (i in 1:10){
  print(rbinom(1,1,1-ruleFollowing))
}
...
df$Feedback[df$Other == 0 & df$Self == 1] <- -1 #self loses on right, goes left
df$Feedback[df$Other == 0 & df$Self == 0] <- -1 #self wins on left, goes left
...
data <- list(
  ...
  strategy_choice = df$Feedback # whether or not agent is choosing right 
)
...
#collect stan model
file <- file.path("matching_pennies_test_no_rate.stan")
...
**TEST: For single game between memory / random**
```{r}
#Defining agent function
MemoryAgent_f <- function(bias, prevRate, noise) {
  choice = rbinom(1, 1, inv_logit_scaled(bias + prevRate*(1/noise)))
  return(choice)
}
...
for (t in seq(trials)){Other[t] <- RandomAgent_f(rate)}
for (i in 2:trials){
  prevRate = cumsum(Other)/i
  Self[i] <- MemoryAgent_f(bias, prevRate, noise)
}

#show
df <- tibble(Self, Other, trial = seq(trials), prevRate)
```
